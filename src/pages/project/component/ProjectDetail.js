      ...
      >
        {
          activeTabKey === 'member' &&
          <Card bordered={false}>
            <h4 style={{ marginBottom: '16px' }}>{`标注情况： ${basicInfo.completeLabelNum}/${basicInfo.allLabelNum}， 剩余：${basicInfo.allLabelNum - basicInfo.completeLabelNum}`}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{`质检情况： ${basicInfo.reviewNum}/${Math.ceil(basicInfo.allLabelNum * basicInfo.checkRate * 0.01)}，剩余：${Math.ceil(basicInfo.allLabelNum * basicInfo.checkRate * 0.01) - basicInfo.reviewNum < 0 ? 0 : Math.ceil(basicInfo.allLabelNum * basicInfo.checkRate * 0.01) - basicInfo.reviewNum}`}</h4>
            <Table
              rowKey="userId"
              columns={columns}
              dataSource={members}
